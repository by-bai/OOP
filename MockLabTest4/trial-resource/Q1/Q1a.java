...
public class Q1a {
    ...
    public static char[] combineTwoCharArrays(char[] a, char[] b) {
        ...
        // if the number of elements in a and b are not the same
        if (a.length != b.length) {
            throw new IllegalInputException("elements in a and b are not the same"); 
        }  
        ...
        char[] finalCombined = new char[combined.size()];
        for (int i = 0; i < finalCombined.length; i++) {
            finalCombined[i] = combined.get(i); 
        }
        
        // System.arraycopy(combined, 0, finalCombined, 0, combined.size()); 
